...
test.describe('Product Discovery', () => {
  ...
  test('Search returns products related to "hammer"', async ({ homePage }) => {
    const searchResults = await homePage.search.searchProduct(
      searchData.hammer
    );

    await homePage.search.searchResultsContain(
      searchResults,
      searchData.hammer
    );
  });

  test('Products are sorted by price in ascending order', async ({
    homePage,
  }) => {
